...
repository from GitHub and look at the **starter** folder.
...
const App = (props) => {
  ...
  return (
    <div>
      ...
      {loading ? <p>Loading...</p> : <p>{ip}</p>}
      <button onClick={getMyIP} disabled={loading}>
        {ip ? "Again" : "Go"}
      </button>
    </div>
  );
...
Begin by importing `useSelector` from the _React Redux_ package.
